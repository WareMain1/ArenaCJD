...
try {
    ...
    $sql = "SELECT e.id_enfrentamiento, e.id_torneo, e.ronda, e.numero_ronda, e.tipo_participante, e.id_usuario_a, e.id_usuario_b, e.id_equipo_a, e.id_equipo_b, e.estado, e.fecha_hora,
    ...
    $sql .= " ORDER BY CASE WHEN e.fecha_hora IS NULL THEN 1 ELSE 0 END, e.fecha_hora ASC, e.numero_ronda ASC, e.orden_ronda ASC";
    $consulta = $conexion->prepare($sql);
    $consulta->execute($parametros);
    responderPublico(['exito' => true, 'enfrentamientos' => $consulta->fetchAll(PDO::FETCH_ASSOC)]);
} catch (Throwable $error) {
    responderPublico(['exito' => false, 'mensaje' => 'No se pudieron cargar los enfrentamientos públicos.'], 500);
}
